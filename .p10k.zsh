# Generated by Powerlevel10k configuration wizard on 2020-04-03 at 22:21 EEST.
...
() {
  ...
  # Color of the anchor directory segments. Anchor  are never shortened. The first
  ...
  function my_git_formatter() {
    ...
    (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${clean}%F{white}⬇ ${VCS_STATUS_COMMITS_BEHIND}"
    # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${clean}%F{white}⬆ ${VCS_STATUS_COMMITS_AHEAD}"
    # ⇠42 if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" ${clean}%F{white}⬅ ${VCS_STATUS_PUSH_COMMITS_BEHIND}"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" "
    # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${clean}%F{white}⮕ ${VCS_STATUS_PUSH_COMMITS_AHEAD}"
    # *42 if have stashes.
